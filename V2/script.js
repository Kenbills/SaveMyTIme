...
async function handleSubmit(e) {
  ...
  try {
    ...
    if (!response.ok) {
        // Try to get text error message from server
...
function renderResults(data) {
  ...
  elements.toolGroupsContainer.innerHTML = data.toolGroups.map((group, groupIdx) => `
    <div class="animate-fade-in-up" style="animation-delay: ${groupIdx * 100}ms">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            ${group.tools.map((tool, toolIdx) => `
               <div class="group relative flex flex-col p-5 rounded-2xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 hover:border-primary-500 dark:hover:border-primary-500 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 h-full">
                  ...
                  <button
                    onclick="openToolModal(${groupIdx}, ${toolIdx})"
 ...
 */
window.openToolModal = function(groupIdx, toolIdx) {
    if (!analysisData || !analysisData.toolGroups) return;
    
    const tool = analysisData.toolGroups[groupIdx].tools[toolIdx];
    if (!tool) return;
